...
from ...modeling_outputs import (
    ...
    MoeCausalLMOutputWithPastAndPreFFNHidden,
    MoeModelOutputWithPast,
    MoeModelOutputWithPastAndPreFFNHidden,
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, layer_mask) -> torch.Tensor:
        """ """
        ...
        if layer_mask is not None:
            mask = torch.zeros(router_logits.size(), dtype=torch.bool).to(router_logits.device)
...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, MoeModelOutputWithPastAndPreFFNHidden]:
        ...
        pre_ffn_hidden = (
            pre_ffn_hidden if pre_ffn_hidden is not None else self.config.pre_ffn_hidden
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if output_router_logits:
                all_router_logits += (layer_outputs[-2 if pre_ffn_hidden else -1],)

            # need modification
            if pre_ffn_hidden and layer_outputs[-1] is not None:
        ...
        return MoeModelOutputWithPastAndPreFFNHidden(
...
class Qwen2MoeForCausalLM(Qwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        return MoeCausalLMOutputWithPastAndPreFFNHidden(
            loss=loss,
            aux_loss=aux_loss,
            logits=logits,
